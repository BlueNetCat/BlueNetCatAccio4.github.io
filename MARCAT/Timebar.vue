          ...
          <div class="accordion-body">
            <div class="container">
              <div class="row align-items-center flex-nowrap">
                <div class="col btn-group" :key="data.name" :id="data.name" @click.prevent="dataClicked" v-for="data in dataType">
...
export default {
  ...
  data () {
    return {
      ...
      baseURL: "https://nrt.cmems-du.eu/thredds/wms/{URLDATATYPE}?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&FORMAT=image%2Fpng&TRANSPARENT=true&LAYERS={LAYERNAME}&COLORSCALERANGE={MINRANGE}%2C{MAXRANGE}&STYLES=boxfill%2Foccam&WIDTH=256&HEIGHT=256&CRS=CRS%3A84&BBOX=-1%2C36%2C9%2C44&TIME=2021-{MONTH}-{DAY}T12%253A00%253A00.000Z"
    }
  },
  methods: {
    ...
    dataClicked: function(event) {
      // Deselect data types in GUI
      Object.keys(this.dataType).forEach(key => {
        this.dataType[key].active = false;
      });
      // Select active data type
      this.dataType[event.currentTarget.id].active = true;
      // Change data type
      this.generateWMSURL();
    },

    // Generate WMS url
    generateWMSURL: function(){
      console.log("hi");
      let tmpURL = this.baseURL;
      // Data type
      let activeDataType; // = this.dataType.find(el => return el.active == true)
      Object.keys(this.dataType).forEach(key => { if (this.dataType[key].active) activeDataType = this.dataType[key] }); // Returns active data type
      tmpURL = tmpURL.replace('{URLDATATYPE}', activeDataType.url);
      tmpURL = tmpURL.replace('{LAYERNAME}', activeDataType.layerName);
      tmpURL = tmpURL.replace('{MINRANGE}', activeDataType.range[0]);
      tmpURL = tmpURL.replace('{MAXRANGE}', activeDataType.range[1]);
